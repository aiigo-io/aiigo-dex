...
export const NetworkTokenSelector = ({ tokens, selectedToken, onSelect = () => {}, className, disabled = false }: NetworkTokenSelectorProps) => {
  const [open, setOpen] = useState(false);

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <div className={`flex items-center justify-center gap-2 border rounded-lg px-3 py-2 cursor-pointer hover:bg-[#1a1a1a] ${className} shrink-0 ${disabled ? 'pointer-events-none' : ''}`}>
