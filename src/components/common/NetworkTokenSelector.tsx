        ...
        <div className='flex flex-col gap-2'>
          {
            tokens.map((token) => (
              <div
                key={token.address}
                className={`flex items-center gap-3 cursor-pointer hover:bg-gray-100 p-2 rounded-lg ${selectedToken.address === token.address ? 'bg-gray-100 border border-gray-200' : ''}`}
                onClick={() => {
                  onSelect(token);
                  setOpen(false);
                }}
